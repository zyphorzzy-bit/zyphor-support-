...
// --- CANAL EXCLUSIVO DE CLIENTES E PREFIXOS ---
client.on(Events.MessageCreate, async m => {
  if (m.author.bot) return;

  // Lógica do Canal Exclusivo de Clientes
  if (config.clientChannelId && m.channel.id === config.clientChannelId) {
    ...
    // Reage com o emoji configurado na mensagem do cliente
...
client.on(Events.InteractionCreate, async int => {
  if (int.isChatInputCommand()) {
    if (int.commandName === 'config') {
      ...
      const embedCfg = new EmbedBuilder()
        ...
        .setDescription("Ajuste os textos, imagens, canais, cargos de Staff e Clientes:")
        .setColor(0x2b2d31);
      ...
      const rowClientChan = new ActionRowBuilder().addComponents(new ChannelSelectMenuBuilder().setCustomId('cfg_client_chan').setPlaceholder('Selecione o Canal Exclusivo dos Clientes'));
      ...
      const rowClient1 = new ActionRowBuilder().addComponents(new RoleSelectMenuBuilder().setCustomId('cfg_client_role_1').setPlaceholder('Selecione o Cargo de Cliente 1').setMinValues(1).setMaxValues(1));
      const rowClient2 = new ActionRowBuilder().addComponents(new RoleSelectMenuBuilder().setCustomId('cfg_client_role_2').setPlaceholder('Selecione o Cargo de Cliente 2').setMinValues(1).setMaxValues(1));

      return int.reply({ embeds: [embedCfg], components: [rowBtns, rowTarget, rowLogs, rowClientChan, rowStaff, rowClient1, rowClient2], ephemeral: true });
    }
  ...
  if (int.isButton() && int.customId === 'cfg_emoji') {
  ...
  if (int.isModalSubmit() && int.customId === 'modal_cfg_emoji') {
  ...
  // --- BOTÕES E MENUS INTERNOS DO TICKET ---
  if (int.isButton()) {
  ...
  // --- SELEÇÃO DO CARGO DE CLIENTE PELA STAFF ---
